...
describe("Testing functionality", function () {
	...
	it("It should delete", function () {
		...
		assert.equal(this.cache.last.key, "e", "Should be 'e'");
		assert.equal(this.cache.size, 4, "Should be '4'");
		assert.equal(this.cache.items.get("e").next, null, "Should be 'null'");
		assert.equal(this.cache.items.get("e").prev.key, "d", "Should be 'd'");
		assert.equal(this.cache.items.get("d").next.key, "e", "Should be 'e'");
		assert.equal(this.cache.items.get("d").prev.key, "c", "Should be 'c'");
		assert.equal(this.cache.items.get("c").next.key, "d", "Should be 'd'");
		assert.equal(this.cache.items.get("c").prev.key, "b", "Should be 'b'");
		assert.equal(this.cache.items.get("b").next.key, "c", "Should be 'c'");
		assert.equal(this.cache.items.get("b").prev, null, "Should be 'null'");
		this.cache.delete("c");
		assert.equal(this.cache.first.key, "b", "Should be 'b'");
		assert.equal(this.cache.last.key, "e", "Should be 'e'");
		...
		assert.equal(this.cache.items.get("e").prev.key, "d", "Should be 'd'");
		assert.equal(this.cache.items.get("d").next.key, "e", "Should be 'e'");
		assert.equal(this.cache.items.get("d").prev.key, "b", "Should be 'b'");
		assert.equal(this.cache.items.get("b").next.key, "d", "Should be 'd'");
		assert.equal(this.cache.items.get("b").prev, null, "Should be 'null'");
		this.cache.delete("e");
		assert.equal(this.cache.first.key, "b", "Should be 'b'");
